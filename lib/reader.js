...
// Modules
import {open as __open, close as __close, read as __read, createReadStream as __createReadStream} from 'node:fs';
import {PassThrough as PassThroughStream, Readable as ReadableStream} from 'node:stream';
...
export class FdReader extends Reader {
	...
	_read(start, length) {
		return new Promise((resolve, reject) => {
			...
			__read(this.fd, buffer, 0, length, start, (err, bytesRead) => {
				if (err) {
					reject(err);
	...
	_createReadStream(start, length) {
		...
		return __createReadStream(null, {start, end: start + length - 1, fd: this.fd, fs: shimmedFs});
	}
}
