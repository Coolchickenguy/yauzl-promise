...
import { decodeBuffer, validateFilename, readUInt64LE } from './utils.js';
import { INTERNAL_SYMBOL } from './shared.js';
import { Reader } from './reader.js';
import { ExtraField, ReadStreamOptions } from './types.js';
...
const MAX_EOCDR_COMMENT_SIZE = 0xFFFF;
...
export default class Zip {
  ...
  isZip64: boolean | null;

  entryCount: number | null;

  footerOffset: number | null;

  centralDirectoryOffset: number | null;

  centralDirectorySize: number | null;
  ...
  /**
	 ...
	 * @param reader - `Reader` to use to access the ZIP
	 * @param size - Size of ZIP file in bytes
	 * @param options - Options
	 */
  constructor(testSymbol: typeof INTERNAL_SYMBOL, reader: Reader, size: number, options: { decodeStrings?: boolean; validateEntrySizes?: boolean; validateFilenames?: boolean; strictFilenames?: boolean; }) {
    assert(
  ...
  /**
	 ...
	 * @async
	 * @returns - Buffer containing EOCDR
	 */
  async _locateEocdr() {
  ...
  /**
	 ...
	 * @param eocdrBuffer - Buffer containing EOCDR
	 * @returns
	 */
  _parseEocdr(eocdrBuffer: Buffer): undefined {
  ...
  async _parseZip64Eocdr() {
    assert(this.footerOffset, 'Missing footer offset state');
  ...
  /**
	 * Get next entry.
	 * @async
	 * @returns - `Entry` object for next entry, or `null` if none remaining
	 */
  async readEntry() {
  ...
  async _readEntry() {
    ...
    assert(this._entryCursor, 'Missing entry cursor state');
    assert(this.footerOffset, 'Missing footer offset state');
    ...
    if (this.decodeStrings && this.validateFilenames) {
      assert(typeof filename === 'string', 'decodeStrings enabled, but got buffer after decoding should have finished');
  ...
  /**
	 ...
	 */
  async readEntries(numEntries: number): Promise<Entry[]> {
    if (numEntries != null) {
  ...
  /**
	 ...
	 */
  async openReadStream(entry: Entry, options: ReadStreamOptions): Promise<ReadableStream> {
  ...
  /**
	 ...
	 * @param isUtf8 - `true` if UTF8
	 * @param copy - `true` to copy buffer rather than slice
	 * @returns - Decoded data
	 */
  _decodeBuffer(buffer: Buffer, start: number, end: number, isUtf8: boolean, copy: boolean): string | Buffer {
    if (!this.decodeStrings) {
